...
## Course Materials
To facilitate your learning, we have provided several files that you will use throughout the course. These files are listed below:

![](./media/Figure 1-1.png) 
...
While it is easier to set up GitHub at the beginning, you can do it at any time during the course.
...
2.	At the root of your ``C:/`` drive, create a folder named **Github**. This folder will contain all your repositories corresponding to your codes. 

3.	Open Github Desktop and click on **Create a New Repository on your local drive…**.
![](./media/Figure 2-2.png) 
4.	Filled the form as follow and click on ”Create Repository”
![](./media/Figure 2-3.png) 

This will create a folder named “Striplog” in the “Github” folder and generate two files:
...
For this course, we will install Python 3.10 because some of the libraries we will use (see 3.4.2 Installing Libraries) require Python 3.10. 
Note that newer versions of Python are available.

1.	Go to the folder provided with this course ``StartCodingToday/Step2/python-3.10.11-amd64.exe``
2.	Run the installer .exe.
3.	Make sure to check the box "Add Python 3.10 to PATH" before clicking "Cuztomize Installation". This step is crucial as it makes Python accessible from the command line.
...
We will install Visual Studio Code (VS Code) that is a free, open-source, powerful code editor that runs on your desktop and is available for Windows, macOS, and Linux. It comes with built-in support for several languages, including Python.
...
::: {.callout-note}
You might hear about Anaconda which is a distribution of Python and R programming languages designed for scientific computing and data science. It simplifies package management and deployment. It is free and was largely use across SRK but as of October 2020, Anaconda's licensing terms have changed, restricting the use of the Anaconda Individual Edition for enterprises with more than 200 employees.
:::
...
- **Search**: Search for keywords and occurences within your repositories.
- **Source Control**: Synchronise your repository with GitHub.
...
Many extensions are available for VS Code, clic on the icon   of the left of the screen, and in the search bar enter:
...
4.	**GitHub repositories**: This extension allows to quickly browse, search, edit , and commit to Github directly from Visual Studio Code.
...
Each time you will want to run a Python script, Visual Studio will ask you to specify the environment. The environment will include all the libraries necessary to run the script. You can update it though the terminal anytime you need to, and we will do it throughout this course. `
...
### Keyboard short cuts in Jupyter Notebook

![](./media/Figure 4-6.png)
...
24.	Let’s print the hearders of the csv tables in order to visualised the parameters that we want to visualised.
...
26.	Now we can assign a the colros to each lithologies:
...
29.	Let’s assign the value for the hole ID, this is a parameters that can be changed as we go.
```{.python}
...
```{.python}
filtered_parameters['MidDepth'] = (filtered_parameters['from'] + filtered_parameters['to']) / 
```
...
The second parameter indicated the type of file, in this case a csv.
:::

In order to visualise the resultat, let go to the terminal and enter:
